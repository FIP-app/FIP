...
class _LoginState extends State<Login> with PageTransitionUtil {
  ...
  void onLogin() async {
    var success = await context.read<AuthProvider>().login(_emailController.text, _pswController.text);
    ...
    if (!success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Jum, sucedió algo con tu login")
          )
      );
    } else {
  ...
  void _submit() {
    if (_form.currentState!.validate()) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Falta validar algo")
          )
      );
    }
  }
